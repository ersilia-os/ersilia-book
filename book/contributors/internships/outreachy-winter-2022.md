...
Please follow the installation [instructions](/broken/pages/xenMKbpAv8JCcpCviZjg). If you have a UNIX machine (Linux or MacOS) you can install Ersilia directly. If you are using a windows machine you will need a Virtual Machine or a Windows Subsystem Linux (WSL).
...
Your second task for the contribution period is to help us debug any issues when running models with the Ersilia Command Line Interface. You can find more information about it on the [Model Usage](/broken/pages/6xshwvUWKSQNePS5mEgc) guide.
...
[Google Colaboratory](/broken/pages/YArmuDPaCBG4MQERmDrY) uses Google's servers (Linux machines) to run the code, which is very convenient to bypass installation issues. To run Google Colab, you only need a Google Account. If you do not have and do not wish to open a Google Account, we can skip this step.All
