...
This command serves a specified model as an API. It enables precalculation fetching and writing from isaura [Precalculation Store](precalculation-store.md)

<table><thead><tr><th width="237">Option / Argument</th><th width="96">Type</th><th width="113">Default Value</th><th>Description</th></tr></thead><tbody><tr><td><code>&#x3C;MODEL_ID></code> (argument)</td><td>String</td><td><em>Required</em></td><td>The model ID to be served. Can either be the eos identifier or the slug identifier.</td></tr><tr><td><code>--port</code> / <code>-p</code></td><td>Integer</td><td><code>None</code></td><td>The port to use when creating a model server. If unspecified, Ersilia looks for empty ports to use on the user's system.</td></tr><tr><td><code>--track</code> / <code>-t</code></td><td>Flag</td><td><code>False</code></td><td>Whether the model's runs should be tracked to monitor model and system performance. This telemetry data can be uploaded to S3 if the appropriate credentials are provided. Currently only Ersilia developers and other Ersilia tools have that privilege.</td></tr><tr><td><code>--tracking-use-case</code></td><td>String</td><td><code>local</code></td><td>If <code>--track</code> is true, this command allows specification of the tracking use case. Current options are: <code>local</code>, <code>hosted</code>, <code>self-service</code> and <code>test</code>.</td></tr><tr><td><code>--enable-cache/--disable-cache</code></td><td>Flag</td><td><code>True</code></td><td>Toggle Redis-based local caching on or off. If it enabled the results from model APIs will be cached for 7 days.</td></tr><tr><td><code>--max-cache-memory-frac</code></td><td>Float</td><td><code>None</code></td><td>Sets the maximum fraction of memory to use by Redis for caching. Recommend value <code>0.2-0.7.</code></td></tr><tr><td><code>--read-store/-rs</code></td><td>Bool</td><td><code>False</code></td><td>Enables precalculation fetching and saving from isaura</td></tr><tr><td><code>--write-store/-ws</code></td><td>Bool</td><td><code>False</code></td><td>Enables precalculation writing and saving to isaura</td></tr><tr><td><code>--access/-a</code></td><td>String</td><td><code>None</code></td><td>Specifies writing access level [either public or private]</td></tr><tr><td><code>--nearest-neighbors/-nn</code></td><td>Bool</td><td>False</td><td>Enables nearest neighbor search for fetching calculations!</td></tr></tbody></table>
...
This command samples inputs for a given model. The command expects a model to be served in the current shell, otherwise a model needs to be explicitly specified. By default, five examples will be sampled for the model and saved to the specified file path. This behavior can be customized with relevant flags.

<table><thead><tr><th width="178.91796875">Option / Argument</th><th width="110">Type</th><th width="137">Default Value</th><th>Description</th></tr></thead><tbody><tr><td><code>model</code> (argument)</td><td>String</td><td><code>None</code></td><td>Optional model ID for which to generate examples, if no model is already served.</td></tr><tr><td><code>--n_samples</code>, <code>-n</code></td><td>Integer</td><td><code>5</code></td><td>Specify the number of example inputs to generate for the given model.</td></tr><tr><td><code>--file_name</code>, <code>-f</code></td><td>String</td><td><em>Required</em></td><td>File name where the examples should be saved.</td></tr><tr><td><code>--mode</code>, <code>-m</code></td><td>String</td><td><code>random</code></td><td>By default Ersilia samples randomly from an ersilia_maintained_inputs list (<code>--mode random</code>). If the <code>deterministic</code> flag is set, it will always provide the same examples (even for different models). The <code>predefined</code> flag forces the sampling from the example file within the model source. Only the number of samples present in the file are returned, even if --n_samples is greater than that number.</td></tr></tbody></table>
...
This command used to fetch cached precalculation from the cloud and the local Redis system depending on the flag specified in the serve command [#id-2.-serve-command](command-line-interface.md#id-2.-serve-command "mention").

{% hint style="warning" %}
Note that fetching a sample size of less than 50,000 inputs from the cloud is not cost-efficient and therefore we do not recommend it.
{% endhint %}
...
{% hint style="info" %}
A note on **session management**. Logs for Ersilia CLI executions are stored in session folders under the `~/eos/sessions` directory. The session identifier is assigned at the terminal level; that is, all runs performed in the same terminal belong to the same session subfolder. Inside each session subfolder, multiple models can exist, corresponding to the sequence of executions in the terminal. When a model is served, the path to the session folder is displayed. When a terminal is closed, the session subfolder is deleted. Therefore, sessions should be treated as temporary, not persistent, subfolders.
{% endhint %}
