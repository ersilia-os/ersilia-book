...
* Async model run check: tests an asynchronous endpoints. Asynchronous endpoints provides an inference capability which can be run in background (more details [here](https://github.com/ersilia-os/ersilia-pack?tab=readme-ov-file#api-documentation)). The model gets served through Ersilia CLI from the specified source and then runs the inference over three endpoint `/job/submit`, `/job/status`, and `/job/result`. The result from the result endpoint will be checked if it contain non null values.
