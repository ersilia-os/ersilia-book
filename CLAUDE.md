...
- **Spelling is genuinely inconsistent** between British and American English across the book (`organisation` ≈ 40 / `organization` ≈ 15; `-ise` / `-ize` mixed; `behavior` exclusively American). **Do not do a global sweep to normalise one way or the other** — it would touch dozens of files and isn't what the user has asked for. Within a single page, match what's already there. If a page is itself inconsistent, you can quietly align it to its dominant spelling while making other edits, but don't open a PR purely for that.
- **Don't rewrite for tone.** The author voice varies between sections (and sometimes between paragraphs) and that variation is fine. In an editorial pass, fix typos, broken links, factual errors, and structural duplication — but leave the prose voice alone unless the user explicitly asks for a rewrite.
